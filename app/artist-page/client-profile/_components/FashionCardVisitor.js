...
// import ThreeDotsDropdown from '../../profile-for-artist/_components/ThreeDotsDropDown'; // Removed
// import CustomButton from '../../../../components/CustomButton'; // Removed
import { useSearchParams, useRouter } from 'next/navigation';
// import AwardUploadModal from '../../profile-for-artist/_components/AwardsUploadModal'; // Removed
// import UploadModal from '../../profile-for-artist/_components/UploadModal'; // Removed
import Link from 'next/link';
// import DeleteConfirmationModal from '../../profile-for-artist/_components/DeleteConfirmationModal'; // Removed

const FashionCardVisitor = () => {
    const [activeTab, setActiveTab] = useState('design');
    const [showAll, setShowAll] = useState(false);
    //   const [deleteModalOpen, setDeleteModalOpen] = useState(false); // Removed
    //   const [selectedDesignId, setSelectedDesignId] = useState(null); // Removed
    ...
    // handleEdit and handleDelete removed

    return (
        <>
            {/* View Visitors Mode button removed */}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {collections.slice(0, visibleItems.design).map((item) => (
                                <div
                                    key={item.id}
                                    className="group rounded-xl overflow-hidden shadow-sm bg-white lg:relative lg:aspect-[4/3]"
                                >
                                    <Link href={`/fashion-designers/${item.id}`} className="block w-full h-full">
                                        {/* Image Container */}
                                        <div
                                            className="relative aspect-[4/3] bg-cover bg-center lg:absolute lg:inset-0 lg:w-full lg:h-full"
                                            style={{ backgroundImage: `url('/dev-images/FashionImg.png')` }}
                                        >
                                            {/* Overlay Gradient - Desktop Only */}
                                            <div className="hidden lg:block absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-90 transition-opacity duration-300" />

                                            {/* View Count - Mobile Only */}
                                            <div className="absolute top-3 left-3 z-10 bg-black/50 rounded-full px-2 py-1 flex items-center gap-1 text-white text-xs lg:hidden">
                                                <Eye size={14} />
                                        ...
                                        {/* Bottom Content */}
                                        <div className="p-4 text-[#222222] lg:absolute lg:bottom-0 lg:left-0 lg:right-0 lg:bg-black/30 lg:backdrop-blur-[2px] lg:text-white lg:opacity-0 lg:group-hover:opacity-100 lg:transition-opacity lg:duration-300">
                                            <div className="flex flex-col mb-1 lg:flex-row lg:justify-between lg:items-center">
                                                <h3 className="font-semibold text-base">{item.title}</h3>
                                                <span className="font-bold text-base">{item.price}</span>
                                            </div>

                                            {/* Desktop Bottom Row (Eye + License) */}
                                            <div className="hidden lg:flex items-center gap-4 text-xs sm:text-sm text-gray-200">
...
export default FashionCardVisitor;
