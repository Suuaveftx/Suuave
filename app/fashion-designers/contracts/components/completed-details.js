...
export default function CompletedDetailsPage({ params }) {
  ...
  return (
    <>
      <PageContainer>
        <ContractHeader title="Contract Information" maxWidth="max-w-6xl" tab="completed" showBack={true} />
                    ...
                    <div className="flex items-center justify-center gap-2 mb-6 text-[#222222]">
                      ...
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <FaStar
                            key={i}
                            className={i < contractData.artist.rating ? "text-yellow-500" : "text-gray-300"}
                          />
                        ))}
                      </div>
                      <span className="text-sm text-[#3A98BB]">
                        ({contractData.artist.reviews}{" "}
                        <Link href="/artist-page/profile-vistor-view?tab=reviews">Reviews</Link>)
                      </span>
                    </div>
                  </div>
                </CardBody>
