/** Shared layout tokens for Fashion Designer & Artist sections */
export const NAVBAR_HEIGHT_PX = 80;
export const NAVBAR_HEIGHT_CLASS = 'h-[80px]';
export const NAVBAR_OFFSET_CLASS = 'pt-[80px]';
export const PAGE_TOP_SPACING_CLASS = 'pt-6 md:pt-7';
export const SECTION_SHELL_CLASS = 'mx-auto bg-[#DBDBDB]/30 min-h-screen w-full overflow-x-hidden';

export const ARTIST_ONBOARDING_ROUTES = [
  '/artist-page/personal-details',
  '/artist-page/edit-profile',
];

export const ARTIST_SELF_LAYOUT_PREFIXES = [
  '/artist-page/profile-vistor-view',
];

export const FASHION_ONBOARDING_ROUTES = [
  '/fashion-designers/personal-details',
];
